...
class iTunesConnect
{
    ...
    private $storeId; // Your app unique ID from iTunes Connect
    private $cookies = [
        'dslang' => 'EN-EN',
        'site'  => 'ENG',
    ];
    public $authSuccess = false;
    public function __construct($userId, $password, $storeId) {
    ...
    public function getStats(\DateTime $startTime, \DateTime $endTime, array $measures = [], $frequency = 'DAY', array $filters = []) {
        $url = parse_url(self::apiUrl);
        $headers = [
            'Cookie' => self::getCookiesString($this->cookies),
            'X-Requested-By' => $url['host'],
        ];
